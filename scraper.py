...
"""
╔══════════════════════════════════════════════════════════╗
║  🌊  AMTEZ WAVE 2026 - AQUA GLASS EDITION  🌊        ║
...
║  💎  Design: Aqua Glass Wave                          ║
║                                                        ║
║  ✨  PREMIUM FEATURES:                                 ║
║     • Parallax Cover متحرك                            ║
║     • Glass Morphism طبقات                             ║
║     • Story Rings حول الصورة                          ║
║     • Animation Entrance                              ║
║     • Music Wave Animation                            ║
║     • Notification Badge نبض                           ║
║     • Bottom Nav Floating                             ║
║     • Glow Effects متقدمة                             ║
║     • علامة توثيق فيروزية                             ║
║     • فيديوهات داخلية                                 ║
║     • غلاف + موقع + بريد                              ║
║     • توثيق وحظر من لوحة الأدمن                       ║
╚══════════════════════════════════════════════════════════╝
"""
...
def build_config():
    return f"""// 🌊 AMTEZ WAVE 2026 - Configuration
...
const COVER_COLORS = {AQUA_COLORS_JS};
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        body{
            min-height:100vh;
            background:radial-gradient(ellipse at top, #164e63, #0a1a20, #000);
        ...
        .card{
            ...
            box-shadow:0 30px 70px rgba(6,182,212,0.15),inset 0 0 30px rgba(6,182,212,0.02);
        ...
        .tab.active{background:linear-gradient(135deg, #06b6d4, #0891b2);color:#fff;box-shadow:0 8px 20px rgba(6,182,212,0.4)}
        ...
        .link a{color:#22d3ee;text-decoration:none;font-weight:600}
    </style>
</head>
<body>
    ...
    <div class="card">
        <div class="logo">🌊</div>
        <h1>AMTEZ</h1>
        <p class="sub">Aqua Wave 2026 Ultra ✨</p>
    ...
    <script>
        ...
        console.log('🌊 AMTEZ Auth Ready');
    </script>
</body>
</html>"""

# ═══════════════════════════════════════════════════════════
# 🌊 3. index.html - الرئيسية (مع Story Rings + إضافات)
# ═══════════════════════════════════════════════════════════
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .toast{
            ...
            background:rgba(10,20,24,0.95);padding:12px 24px;border-radius:50px;
        ...
        .overlay{
            position:fixed;inset:0;background:rgba(10,20,24,0.97);
            backdrop-filter:blur(40px);z-index:400;overflow-y:auto;
        }
        .overlay-header{
            display:flex;justify-content:space-between;align-items:center;
            padding:16px;border-bottom:1px solid var(--border);
            position:sticky;top:0;background:rgba(10,20,24,0.8);
        }
        .btn-close{
...
<div id="mainApp">
    <div class="topbar">
        <div style="display:flex;align-items:center">
            ...
            <span class="logo-text">AMTEZ</span>
        </div>
        <div class="tabs">
    ...
    <div class="videos-wrap" id="videosWrap">
        <div style="display:flex;align-items:center;justify-content:center;height:100vh;color:rgba(255,255,255,0.5);flex-direction:column;gap:12px">
            <i class="fas fa-water" style="font-size:48px;opacity:0.3;color:#06b6d4"></i>
...
<script>
    ...
    auth.onAuthStateChanged(async (user) => {
        ...
        db.ref('notifications/' + user.uid).on('value', s => {
            ...
            if (badge) badge.style.display = Object.keys(ns).length > 0 ? 'block' : 'none';
        });
    ...
    function renderVideos() {
        ...
        if (!filtered.length) {
            container.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100vh;color:rgba(255,255,255,0.5);flex-direction:column;gap:12px"><i class="fas fa-water" style="font-size:48px;opacity:0.3;color:#06b6d4"></i><p>${currentFeed === 'forYou' ? 'لا توجد فيديوهات بعد' : 'تابع مستخدمين لرؤية فيديوهاتهم'}</p></div>`;
            return;
        }
        container.innerHTML = '';
    ...
    function openSearch() {
        ...
        window.doSearch = function() {
            ...
            resultsDiv.innerHTML = `${users.length ? `<div style="margin-bottom:16px"><h4 style="font-size:12px;opacity:0.5;margin-bottom:8px">👥 مستخدمين</h4>${users.map(u => `<div onclick="openUserProfile('${u.uid || Object.keys(allUsers).find(k=>allUsers[k]===u)}')" style="display:flex;align-items:center;gap:10px;padding:10px;cursor:pointer;border-bottom:1px solid rgba(6,182,212,0.1)"><img src="${u.avatarUrl || (DICEBEAR_URL + '?seed=' + (u.uid || u.username))}" style="width:40px;height:40px;border-radius:50%"><div>@${u.username} ${u.isVerified ? '<span class="verified-badge-main">✓</span>' : ''}</div></div>`).join('')}</div>` : ''}${vids.length ? `<div><h4 style="font-size:12px;opacity:0.5;margin-bottom:8px">🎬 فيديوهات</h4>${vids.map(v => `<div onclick="window.open('${v.url}', '_blank')" style="display:flex;align-items:center;gap:10px;padding:10px;cursor:pointer;border-bottom:1px solid rgba(6,182,212,0.1)"><i class="fas fa-play-circle" style="color:#06b6d4;font-size:20px"></i><span style="font-size:13px">${(v.description || 'فيديو').substring(0, 40)}</span></div>`).join('')}</div>` : ''}${!users.length && !vids.length ? '<div style="text-align:center;opacity:0.5;padding:30px">لا توجد نتائج</div>' : ''}`;
        };
        setTimeout(() => { const input = document.getElementById('searchQ'); if (input) input.focus(); }, 300);
    }

    function openNotifs() {
        showOverlay('🔔 الإشعارات', `<div style="text-align:center;opacity:0.5;padding:40px"><i class="fas fa-bell" style="font-size:48px;color:#06b6d4;margin-bottom:12px;display:block"></i><p>لا توجد إشعارات حالياً</p></div>`);
    }
    ...
    console.log('🌊 AMTEZ Index Ready ✨');
</script>
</body>
</html>"""

# ═══════════════════════════════════════════════════════════
# 🌊 4. profile.html - الملف الشخصي (مع Parallax Cover + Glass Layers)
# ═══════════════════════════════════════════════════════════
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
    <title>🌊 AMTEZ | ملف شخصي</title>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-app-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-database-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-auth-compat.js"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <style>
        :root{--glass:rgba(6,182,212,0.04);--border:rgba(6,182,212,0.15);--accent:#06b6d4;--accent2:#22d3ee;--bg:#0a1418;--card:rgba(6,182,212,0.06)}
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .cover-gradient{position:absolute;inset:0;background:linear-gradient(to bottom, transparent 30%, rgba(10,20,24,0.4) 60%, rgba(10,20,24,0.95) 100%);pointer-events:none;z-index:1}
        .cover-glow{position:absolute;inset:0;background:radial-gradient(ellipse at center, rgba(6,182,212,0.2) 0%, transparent 70%);pointer-events:none;z-index:2}
        ...
        /* 🌊 STORY RING AVATAR */
        .avatar-lg{width:120px;height:120px;border-radius:50%;overflow:hidden;cursor:pointer;background:linear-gradient(135deg, #06b6d4, #22d3ee, #67e8f9);padding:3px;box-shadow:0 0 30px rgba(6,182,212,0.4), 0 0 60px rgba(6,182,212,0.1);animation:avatarGlow 3s ease-in-out infinite}
        ...
        .btn-primary{background:linear-gradient(135deg,var(--accent),var(--accent2));border:none;font-weight:700;box-shadow:0 8px 25px rgba(6,182,212,0.4)}
        ...
        .thumb{aspect-ratio:9/16;background:rgba(6,182,212,0.05);border-radius:8px;cursor:pointer;position:relative;overflow:hidden;transition:transform 0.3s}
        .thumb:hover{transform:scale(1.03);z-index:1;box-shadow:0 0 25px rgba(6,182,212,0.2)}
        .thumb video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}
        .thumb .play-count{position:absolute;bottom:6px;left:6px;font-size:10px;background:rgba(0,0,0,0.7);backdrop-filter:blur(5px);padding:3px 8px;border-radius:10px;z-index:2}
        ...
        .toast-msg{position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:rgba(10,20,24,0.95);padding:12px 24px;border-radius:30px;z-index:300;border:1px solid rgba(6,182,212,0.3);font-size:13px;opacity:0;transition:opacity 0.3s;pointer-events:none;white-space:nowrap;box-shadow:0 8px 32px rgba(0,0,0,0.4)}
        .toast-msg.show{opacity:1}

        /* 🌊 ADMIN PANEL */
        .admin-panel{background:transparent;border:none;padding:0 8px;margin:0 8px 100px 8px;font-family:'Segoe UI',sans-serif}
        .admin-panel h3{color:#22d3ee;font-size:20px;margin-bottom:20px;display:flex;align-items:center;gap:10px;font-weight:700}
        ...
        .btn-verify{background:linear-gradient(135deg, #06b6d4, #0891b2);color:#fff;box-shadow:0 4px 12px rgba(6,182,212,0.3)}
...
<div id="content" style="display:none">
    ...
    <div class="section-title"><i class="fas fa-water" style="color:var(--accent)"></i> الفيديوهات</div>
...
<script>
    ...
    async function loadProfile() {
        ...
        const aquaVerifiedBadge = u.isVerified ? '<span class="badge-verified">✓</span>' : '';
        document.getElementById('nameDisplay').innerHTML = '@' + (u.username || 'مستخدم') + ' ' + aquaVerifiedBadge;
        ...
        if (!uvs.length) {
            ...
            uvs.forEach(v => {
                ...
                d.className = 'thumb';
                d.innerHTML = `<video src="${v.url}" muted loop playsinline preload="metadata"></video><span class="play-count">❤️ ${v.likes || 0}</span>`;
                d.onclick = () => { const video = d.querySelector('video'); if (video.paused) { video.play(); } else { video.pause(); } };
                g.appendChild(d);
            });
        }
    ...
    async function loadAdminPanel() {
        ...
        adminDiv.innerHTML = `
            <h3><i class="fas fa-shield-haltered"></i> لوحة تحكم الأدمن</h3>
            <div class="admin-stats-grid">
                ...
                <div class="stat-card"><div class="stat-icon" style="background: linear-gradient(135deg, #f59e0b, #d97706);"><i class="fas fa-video"></i></div><div class="stat-info"><h4>فيديوهات</h4><span>${totalVideos}</span></div></div>
                <div class="stat-card"><div class="stat-icon" style="background: linear-gradient(135deg, #22d3ee, #06b6d4);"><i class="fas fa-check-circle"></i></div><div class="stat-info"><h4>موثقين</h4><span>${totalVerified}</span></div></div>
    ...
    async function loadAdminUsersList() {
        ...
        listContainer.innerHTML = usersArray.map(([id, u]) => {
            ...
            const verifiedBadgeHtml = isVerified ? '<span class="badge-verified">✓</span>' : '';
            let actionBtns = '';
            if (isBanned) {
                actionBtns = `<button class="admin-btn btn-unban" onclick="toggleBanUser('${id}')"><i class="fas fa-undo"></i> إلغاء الحظر</button>`;
            } else {
                actionBtns = `
                    <button class="admin-btn btn-verify ${isVerified ? 'unverify' : ''}" onclick="toggleVerifyUser('${id}')">
                        ${isVerified ? '<i class="fas fa-times-circle"></i> إلغاء' : '<i class="fas fa-check-circle"></i> توثيق'}
                    </button>
                    <button class="admin-btn btn-ban" onclick="toggleBanUser('${id}')"><i class="fas fa-ban"></i> حظر</button>`;
            }
            return `<div class="admin-user-item" onclick="if(event.target.tagName !== 'BUTTON' && event.target.tagName !== 'I') openUserProfile('${id}')" style="cursor: pointer;">
                    <div class="admin-user-info"><div class="admin-avatar"><img src="${avatar}" alt="avatar"></div><div class="admin-user-details"><h4>@${username} ${verifiedBadgeHtml}</h4><p>${email}</p></div></div>
                    <div class="admin-user-actions" onclick="event.stopPropagation();">${actionBtns}</div></div>`;
        }).join('');
    }
    ...
    console.log('🌊 AMTEZ Profile v5 Ready ✨');
</script>
</body>
</html>"""
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
    <title>🌊 AMTEZ | رفع فيديو</title>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-app-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-database-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-auth-compat.js"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <style>
        :root{--glass:rgba(6,182,212,0.03);--border:rgba(6,182,212,0.12);--accent:#06b6d4;--accent2:#22d3ee;--bg:#0a1418}
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:'Segoe UI',sans-serif;background:var(--bg);color:#fff;min-height:100vh;overflow-y:auto}
        .header{display:flex;align-items:center;gap:12px;padding:16px;border-bottom:1px solid var(--border);background:rgba(10,20,24,0.8);backdrop-filter:blur(20px);position:sticky;top:0;z-index:10}
        .btn-back{background:rgba(6,182,212,0.1);border:1px solid var(--border);width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;cursor:pointer;font-size:16px;transition:all 0.3s}
        .btn-back:hover{background:rgba(6,182,212,0.2);box-shadow:0 0 20px rgba(6,182,212,0.3)}
        .container{max-width:500px;margin:0 auto;padding:20px}
        .dropzone{border:2px dashed rgba(6,182,212,0.3);border-radius:20px;padding:50px 20px;text-align:center;cursor:pointer;background:var(--glass);margin-bottom:20px;transition:all 0.3s}
        .dropzone:hover{border-color:var(--accent);box-shadow:0 0 30px rgba(6,182,212,0.2)}
        ...
        .form-card{background:rgba(6,182,212,0.03);border:1px solid var(--border);border-radius:20px;padding:20px;backdrop-filter:blur(10px)}
        ...
        .progress-fill{background:linear-gradient(90deg,var(--accent),var(--accent2));height:100%;border-radius:30px;width:0%;transition:width 0.3s;box-shadow:0 0 10px rgba(6,182,212,0.5)}
        .progress-text{text-align:center;font-size:12px;margin-top:6px;color:#22d3ee}
        .btn-upload{width:100%;padding:14px;background:linear-gradient(135deg,var(--accent),var(--accent2));border:none;border-radius:30px;color:#fff;font-weight:700;font-size:15px;cursor:pointer;margin-top:16px;box-shadow:0 10px 25px rgba(6,182,212,0.4);transition:all 0.3s}
        .btn-upload:hover:not(:disabled){transform:translateY(-2px);box-shadow:0 15px 35px rgba(6,182,212,0.6)}
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .header{display:flex;align-items:center;gap:12px;padding:16px;border-bottom:1px solid var(--border);background:rgba(10,20,24,0.8);backdrop-filter:blur(20px)}
        ...
        .bubble.sent{background:linear-gradient(135deg,var(--accent),var(--accent2));align-self:flex-end;box-shadow:0 4px 15px rgba(6,182,212,0.3)}
        ...
        .input-bar{display:flex;gap:10px;padding:12px;background:rgba(10,20,24,0.95);backdrop-filter:blur(20px);border-top:1px solid var(--border);align-items:center}
        ...
        .btn-icon{width:42px;height:42px;background:rgba(6,182,212,0.1);border:1px solid var(--border);border-radius:50%;color:#fff;cursor:pointer;font-size:18px;display:flex;align-items:center;justify-content:center;transition:all 0.3s}
        .btn-icon:hover{background:rgba(6,182,212,0.2);box-shadow:0 0 15px rgba(6,182,212,0.3)}
        .btn-send{width:42px;height:42px;background:linear-gradient(135deg,var(--accent),var(--accent2));border:none;border-radius:50%;color:#fff;cursor:pointer;font-size:18px;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 15px rgba(6,182,212,0.4)}
        .btn-send:hover{box-shadow:0 8px 25px rgba(6,182,212,0.6)}
        .conv-item{display:flex;align-items:center;gap:12px;padding:14px;border-bottom:1px solid var(--border);cursor:pointer;transition:all 0.2s}
        ...
        .online-indicator{width:10px;height:10px;background:#22c55e;border-radius:50%;display:inline-block;margin-left:6px;box-shadow:0 0 8px rgba(34,197,94,0.6)}
        ...
        @keyframes spin{to{transform:rotate(360deg)}}
        .toast-msg{position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:rgba(10,20,24,0.95);padding:10px 22px;border-radius:30px;z-index:300;border:1px solid rgba(6,182,212,0.3);font-size:13px;opacity:0;transition:opacity 0.3s;pointer-events:none}
        .toast-msg.show{opacity:1}
    </style>
</head>
<body>
...
<head>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:'Segoe UI',sans-serif;background:var(--bg);color:#fff;min-height:100vh;overflow-y:auto}
        .header{display:flex;align-items:center;gap:12px;padding:16px;border-bottom:1px solid var(--border);position:sticky;top:0;background:rgba(10,20,24,0.8);backdrop-filter:blur(20px);z-index:10}
        .btn-back{background:rgba(6,182,212,0.1);border:1px solid var(--border);width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;cursor:pointer;font-size:16px}
        h2{font-size:18px;font-weight:700}
        .grid{display:grid;grid-template-columns:repeat(3,1fr);gap:2px;padding:2px}
        .thumb{aspect-ratio:9/16;background:rgba(6,182,212,0.05);display:flex;align-items:center;justify-content:center;cursor:pointer;position:relative;overflow:hidden;transition:all 0.3s}
        .thumb:hover{transform:scale(1.03);z-index:1;box-shadow:0 0 25px rgba(6,182,212,0.2)}
        ...
        .thumb .views{position:absolute;bottom:4px;left:4px;font-size:10px;background:rgba(0,0,0,0.6);backdrop-filter:blur(5px);padding:2px 6px;border-radius:10px}
        .spinner{width:32px;height:32px;border:3px solid rgba(6,182,212,0.2);border-top-color:var(--accent);border-radius:50%;animation:spin 0.7s linear infinite;margin:30px auto}
        @keyframes spin{to{transform:rotate(360deg)}}
    </style>
</head>
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
    <title>🌊 AMTEZ | إشعارات</title>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-app-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-database-compat.js"></script>
    <script src="https://www.gstatic.com/firebasejs/10.7.0/firebase-auth-compat.js"></script>
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" rel="stylesheet">
    <style>
        :root{--glass:rgba(6,182,212,0.03);--border:rgba(6,182,212,0.12);--accent:#06b6d4;--bg:#0a1418}
        *{margin:0;padding:0;box-sizing:border-box}
        body{font-family:'Segoe UI',sans-serif;background:var(--bg);color:#fff;min-height:100vh;overflow-y:auto}
        .header{display:flex;align-items:center;gap:12px;padding:16px;border-bottom:1px solid var(--border);position:sticky;top:0;background:rgba(10,20,24,0.8);backdrop-filter:blur(20px);z-index:10}
        .btn-back{background:rgba(6,182,212,0.1);border:1px solid var(--border);width:38px;height:38px;border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;cursor:pointer;font-size:16px}
        .notif-item{display:flex;gap:12px;padding:14px 16px;border-bottom:1px solid var(--border);align-items:center;transition:background 0.2s}
        .notif-item:hover{background:rgba(6,182,212,0.03)}
...
<script>
    ...
    async function loadNotifs(){
        ...
        if(!items.length){c.innerHTML='<div style="text-align:center;opacity:0.5;padding:40px"><i class="fas fa-bell" style="font-size:48px;color:#06b6d4;margin-bottom:12px;display:block"></i><p>لا توجد إشعارات</p></div>';return}
...
║                                                          ║
║  ✨  PREMIUM: Parallax + Glass + Stories + Glow       ║
║                                                          ║
╚══════════════════════════════════════════════════════════╝
    """)
...
{'='*60}
  ...
  📁 الملفات:
     ...
     3. index.html           → الرئيسية + Story Rings + Music Wave
     4. profile.html         → Parallax Cover + Glass Morphism
     5. upload.html          → رفع فيديو
     6. chat.html            → دردشة متطورة
     7. explore.html         → استكشاف
     8. notifications.html   → إشعارات
     9. settings.html        → إعدادات

  🌊 الإضافات المبهرة:
     • 🖼️ Parallax Cover متحرك
     • 💎 Glass Morphism ثلاثي الطبقات
     • 📊 Story Rings حول الصورة
     • ✨ Animation Entrance
     • 🎵 Music Wave Animation
     • 🔔 Notification Badge بنبض
     • 📱 Floating Bottom Nav
     • 🌟 Glow Effects متقدمة
     • 💬 Glass Contact Cards
     • 🛡️ لوحة أدمن كاملة
